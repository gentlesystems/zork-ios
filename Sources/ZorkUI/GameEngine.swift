...
final class GameEngine {
    ...
    /// When on, the terminal font shrinks to fit Zork's 60+ column lines on the
    /// current device width. When off, the terminal honors the system's Dynamic
    /// Type body size so users can rely on iOS-wide text-size accessibility.
    var adaptiveSizingEnabled: Bool = UserDefaults.standard.object(forKey: "zorkAdaptiveSizing") as? Bool ?? true {
        didSet { UserDefaults.standard.set(adaptiveSizingEnabled, forKey: "zorkAdaptiveSizing") }
    }
    ...
    private func copyDataFileIfNeeded() {
        ...
        // Found in the host app's main bundle (Copy Bundle Resources phase).
